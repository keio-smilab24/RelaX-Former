...
def create_np(database):
    ...
    output_file = "data/ltrpo_dataset/ltrpo_database_np.json"
...
def convert_jpg_to_npy_2d(file_name):
    return file_name.replace("EXTRACTED_IMGS_", "image_features_2d").replace("jpg", "npy")
...
def create_full_image_feature(database, enable_clip=False, enable_sam=False, enable_ViT=False, enable_llava=False):
    ...
    if enable_llava:
        ...
        llava_model.to("cuda:0")  # type: ignore
    ...
    for data in database:
        ...
        for path in data["full_image_feature_path"]:
            image_path_instruction_pair = (convert_npy_to_jpg(path), instruction)
    ...
    for img_path, instruction in tqdm(sorted(list(image_path_set))):
        ...
        if enable_llava:
            ...
            llava_npy_path = convert_jpg_to_npy(img_path).replace("features", "llava_features_latent_full_bit")
            llava_dir = os.path.dirname(llava_npy_path)
            os.makedirs(llava_dir, exist_ok=True)
            np.save(llava_npy_path, llava_feature)

        if enable_clip or enable_sam or enable_ViT:
            if enable_clip:
...
def create_full_image_feature_2d(database):
    ...
    for img_path in tqdm(sorted(list(image_path_set))):
        ...
        npy_path = convert_jpg_to_npy_2d(img_path)
        dir = "/".join(npy_path.split("/")[:-1])
        os.makedirs(dir, exist_ok=True)
        np.save(npy_path, output_data)
...
def create_dataset_per_env(database):
    ...
    # ltrpo_{split}_{env}.json
    # ltrpo_{split}_id2path.json
    envs_dict = {
        "val_unseen": ["oLBMNvg9in8", "zsNo4HB9uLZ"],
        "test": ["QUCTc6BB5sX", "X7HyMhZNoso"],
        "train": ["train"],
    }
    ...
    for split in envs_dict.keys():
        ...
        for env in envs_dict[split]:
            # create ltrpo_{split}_{env}.json
            env_dataset = []
            for data in database:
                if split == "train":
                    if (
                        data["environment"] not in envs_dict["val_unseen"]
                        and data["environment"] not in envs_dict["test"]
                    ):
            ...
            # output ltrpo_{split}_{env}.json
            output_file = f"data/ltrpo_dataset/ltrpo_{split}_{env}.json"
            with open(output_file, "w") as wf:
                json.dump(sorted(env_dataset, key=lambda x: x["instruction_id"]), wf, indent=2)

            # create ltrpo_{split}_id2path.json
            output_by_env = {}
            ...
            if split != "train":
                ...
                for bbox_id in sorted(output_by_env.keys()):
                    ...
                    # gpt4v embeddings
                    gpt4v_embeddings = np.load(output_by_env[bbox_id]["gpt4v_embeddings"])
                    gpt4v_features.append(gpt4v_embeddings)
                ...
                os.makedirs("data/eval_features", exist_ok=True)
                base_path = f"data/eval_features/eval_features_{split}_{env}"
        ...
        # 定性的結果で必要
        # output ltrpo_{split}_id2path.json
        output_file = f"data/ltrpo_dataset/ltrpo_{split}_id2path.json"
        with open(output_file, "w") as wf:
...
def create_pseudo_gt_with_llava(database, cos_similarity_data):
    """
    Add pseudo gt to the database with LLaVA captions
    """
    ...
    for instruction_obj in tqdm(cos_similarity_data["output"]):
        ...
        for img_id in top20_img_ids:
            ...
            env_name, waypoint_name, id = img_id_parts[1], img_id_parts[2], img_id_parts[3]
            img_path = f"data/EXTRACTED_IMGS_/{env_name}/{waypoint_name}/{id}.jpg"
            try:
                img = Image.open(img_path)
            except FileNotFoundError:
                print(f"Image not found: {img_path} for instruction: {instruction_parts}")
                continue
            target_or_destination = (
                data["instruction_chatgpt"] if data["mode"] == "<target>" else data["llm_data"]["destination"]
            )
        ...
        output.append(data)
    with open("data/ltrpo_dataset/ltrpo_pseudo_gt.json", "w") as wf:
    ...
    for split in envs_dict.keys():
        ...
        for env in envs_dict[split]:
            env_dataset = [
                data
                for data in database
                if data["environment"] not in reference_env_dict["val_unseen"]
                and data["environment"] not in reference_env_dict["test"]
            ]
            ...
            for batch in range(total_batches):
                ...
                output_file = f"data/ltrpo_dataset/ltrpo_{split}_{env}_batch{batch}.json"
                with open(output_file, "w") as wf:
                ...
                output_data = {"bboxId_list": bboxId_list}
                base_path = f"data/eval_features/eval_features_{split}_{env}_batch{batch}"
                np.save(f"{base_path}_full.npy", np.array(image_features))
                np.save(f"{base_path}_full_2d.npy", np.array(image_features_2d))
                np.save(f"{base_path}_llava.npy", np.array(llava_features))
                np.save(f"{base_path}_gpt4v_embeddings.npy", np.array(gpt4v_features))
        ...
        # Output id2path JSON for the entire split, not by batches
        output_file = f"data/ltrpo_dataset/ltrpo_{split}_id2path.json"
        with open(output_file, "w") as wf:
            json.dump(output_by_split, wf, indent=2)
...
def interpolate_features(base_database, target_database):
    ...
    open("data/ltrpo_dataset/__ltrpo_pseudo_gt.json", "w").write(json.dumps(target_database, indent=2))


# poetry run python src/create_dataset_for_switching.py --np --full --dataset --full_2d --baseline_dataset --sam
def main():
    ...
    dataset_file = "data/ltrpo_dataset/ltrpo_database.json"
    with open(dataset_file, "r") as f:
        database = json.load(f)

    if args.use_np_cached:
        with open("data/ltrpo_dataset/ltrpo_database_np.json", "r") as f:
            database = json.load(f)
    ...
    if args.pseudo_gt_with_llava:
        cos_similarity_data = "./result/mp3d_cos_similarity_clip_20_without_similarity.json"
    ...
    if args.interpolate:
        interpolate_features(
            json.load(open("data/ltrpo_dataset/ltrpo_database_20_pseudo_gt.json", "r")),
            json.load(open("data/ltrpo_dataset/ltrpo_database.json", "r")),
        )

    # if args.pad:
    #     pad_alpha_clip_features("data/image_alpha_clip_visual_features")
